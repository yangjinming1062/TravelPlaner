...
const SingleDestinationResult = () => {
  const { taskId } = useParams<{ taskId: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  const { data: planResult, isLoading, isError, error } = useSinglePlanResult(taskId || "");
  ...
  const handleFavoriteToggle = () => {
    ...
    updateFavorite(
      { taskType: "single", taskId, data: { is_favorite: !isFavorite } },
      {
        onSuccess: () => {
                ...
                {planResult.highlights.map((highlight, index) => (
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    {highlight.best_visit_time && (
                      <p className="text-xs mt-2 text-primary">最佳游览时间: {highlight.best_visit_time}</p>
                    )}
                  </div>
                ))}
              </div>
...
export default SingleDestinationResult;