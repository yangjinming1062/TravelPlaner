...
import { DailyPlanSchema, ActivityPlanSchema, AccommodationPlanSchema } from '@/types/planning';
    ...
    <Card className="mb-4">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant="outline">第{new Date(dailyPlan.date).getDate()}天</Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {/* 路线信息（可选） */}
        {showRouteInfo && dailyPlan.route_plan && dailyPlan.route_plan.length > 0 && (
          <>
            <Separator />
...
// 住宿信息组件
const AccommodationInfo: React.FC<{ accommodation: AccommodationPlanSchema }> = ({ 
  accommodation 
}) => (
...
// 活动项组件
const ActivityItem: React.FC<{ activity: ActivityPlanSchema }> = ({ activity }) => (
  <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
    ...
    <div className="flex-grow">
      ...
      {activity.description && (
        <div className="text-sm text-green-700 mt-1">{activity.description}</div>
      )}
    </div>
  </div>
);
