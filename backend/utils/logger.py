...
def get_logger(module: str = None, filename: str = None, format: str = None):
    """
    获取一个配置好的logger实例
    ...
    # region 计算日志目录与文件输出开关
    log_dir = None
    if CONFIG.log_dir:
        base_dir = Path(CONFIG.log_dir)
        log_dir = base_dir / module if module else base_dir
        log_dir.mkdir(parents=True, exist_ok=True)
    # endregion

    # 创建格式化器
    formatter = logging.Formatter(format or CONFIG.log_format, datefmt=CONFIG.log_date_format)

    # region 配置文件处理器（当启用文件日志时）
    if CONFIG.log_dir:
        # 如果提供了文件名，使用单一文件模式
        if filename:
            file_handler = FileHandler(str(log_dir / f"{filename}.log"), encoding="utf-8", mode="a")
            file_handler.setFormatter(formatter)
            new_logger.addHandler(file_handler)
        else:
            # 使用分离文件模式（按照日志级别分开）
            info_handler = FileHandler(str(log_dir / CONFIG.log_info_name), encoding="utf-8", mode="a")
            info_handler.setFormatter(formatter)
            info_handler.setLevel(getattr(logging, CONFIG.log_level))
            # 只处理INFO和DEBUG级别
            info_handler.addFilter(lambda record: record.levelno <= logging.INFO)
            new_logger.addHandler(info_handler)

            # 配置ERROR处理程序
            error_handler = FileHandler(str(log_dir / CONFIG.log_error_name), encoding="utf-8", mode="a")
