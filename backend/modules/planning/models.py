...
class PlanningTaskBase(ModelBase, ModelTimeColumns):
    ...
    status: Mapped[str] = mapped_column(String(16), default="pending", comment="规划状态: pending, processing, completed, failed")
...
class PlanningRouteTask(PlanningTaskBase, PlanningPreferenceColumns):
    ...
    max_stopover_duration: Mapped[int] = mapped_column(default=2, comment="计划停留时长：每个途径点最多游玩多久（小时）")
    route_preference: Mapped[str] = mapped_column(default="平衡", comment="路线偏好：速度优先, 风景优先, 经济优先, 平衡")
